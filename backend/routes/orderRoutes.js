...
const parseQuantityInQuintals = (qtyStr) => {
    ...
    // Priority 1: Check if it says KG, convert to Quintal (More precise for user's formula)
    if (str.toLowerCase().includes('kg')) {
        const kgMatch = str.match(/([\d.]+)/);
        return (parseFloat(kgMatch ? kgMatch[1] : str) || 0) / 100;
    }

    // Priority 2: Look for (XX Quintal) pattern
    const quintalMatch = str.match(/\(?([\d.]+)\s*Quintal\)?/i);
    if (quintalMatch) {
        return parseFloat(quintalMatch[1]) || 0;
    }
    
    // Priority 3: Fallback: Check if it just says Quintal
