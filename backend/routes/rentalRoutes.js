...
router.patch('/:id/status', protect, async (req, res) => {
    try {
        ...
        // Check for conflicts if marking as Accepted (Only for non-kattha bookings)
        if (status === 'Accepted' && previousStatus !== 'Accepted' && rental.priceType !== 'kattha') {
            const overlaps = await Rental.find({
                _id: { $ne: rental._id },
                machine: rental.machine,
                status: { $in: ['Accepted', 'In Progress'] },
                priceType: { $ne: 'kattha' }, // Ignore kattha bookings for overlap
                $or: [
...
router.post('/book', protect, async (req, res) => {
    try {
        ...
        // Check for double booking (Only block if not kattha and another booking is already Accepted or In Progress)
        if (priceType !== 'kattha') {
            const overlaps = await Rental.find({
                machine: machineId,
                status: { $in: ['Accepted', 'In Progress'] },
...
router.get('/check-availability', protect, async (req, res) => {
    try {
        const { machineId, fromDate, toDate, priceType } = req.query;
        ...
        if (priceType === 'kattha') {
            return res.json({ available: true, bookedSlots: [] });
        }
