...
// PIN Management Routes (At Top to avoid conflicts)
...
router.post('/withdraw/request-otp', protect, checkApprovedKsp, async (req, res) => {
  try {
    ...
    if (!allowedStatuses.includes(farmer.status)) {
      return res.status(403).json({ error: 'This account is currently inactive, suspended or blocked. Withdrawal not allowed.' });
    }
...
// Recharge: Check PIN exists
router.get('/recharge/check-pin', protect, async (req, res) => {
  console.log('[KSP-DEBUG] Reached check-pin route for user:', req.user.id);
  try {
    const ksp = await User.findById(req.user.id).select('kspPin');
    if (!ksp) return res.status(404).json({ error: 'KSP not found' });
    res.json({ hasPin: !!(ksp.kspPin && ksp.kspPin.length > 0) });
  } catch (error) {
    res.status(500).json({ error: 'Failed to check PIN' });
  }
});

// Recharge: Set KSP PIN (first time)
router.post('/recharge/set-pin', protect, checkApprovedKsp, async (req, res) => {
  try {
    const { pin } = req.body;
    if (!pin || pin.length !== 4 || !/^\d{4}$/.test(pin)) {
      return res.status(400).json({ error: '4-digit numeric PIN required' });
    }
    const ksp = await User.findById(req.user.id);
    if (!ksp) return res.status(404).json({ error: 'KSP not found' });
    if (ksp.kspPin && ksp.kspPin.length > 0) {
      return res.status(400).json({ error: 'PIN already set. Use update-pin to change it.' });
    }
    const salt = await bcrypt.genSalt(10);
    ksp.kspPin = await bcrypt.hash(pin, salt);
    await ksp.save();
    res.json({ success: true, message: 'KSP PIN set successfully!' });
  } catch (error) {
    console.error('KSP Set PIN Error:', error);
    res.status(500).json({ error: 'Failed to set PIN' });
  }
});

// Recharge: Update/Change KSP PIN
router.post('/recharge/update-pin', protect, checkApprovedKsp, async (req, res) => {
  try {
    const { oldPin, newPin } = req.body;
    if (!oldPin || !newPin || newPin.length !== 4 || !/^\d{4}$/.test(newPin)) {
      return res.status(400).json({ error: 'Old PIN and 4-digit new PIN are required' });
    }
    const ksp = await User.findById(req.user.id);
    if (!ksp) return res.status(404).json({ error: 'KSP not found' });
    if (!ksp.kspPin) {
      return res.status(400).json({ error: 'No PIN set yet. Use set-pin first.' });
    }
    const isMatch = await bcrypt.compare(oldPin, ksp.kspPin);
    if (!isMatch) {
      return res.status(400).json({ error: 'Current PIN is incorrect' });
    }
    const salt = await bcrypt.genSalt(10);
    ksp.kspPin = await bcrypt.hash(newPin, salt);
    await ksp.save();
    res.json({ success: true, message: 'KSP PIN updated successfully!' });
  } catch (error) {
    console.error('KSP Update PIN Error:', error);
    res.status(500).json({ error: 'Failed to update PIN' });
  }
});

// Recharge: Confirm with KSP PIN (replaces OTP)
router.post('/recharge/confirm', protect, checkApprovedKsp, async (req, res) => {
  try {
    const { userId, amount, kspPin } = req.body;
    const amt = parseFloat(amount);
    ...
    if (!allowedStatuses.includes(targetUser.status)) {
      return res.status(403).json({ error: 'This account is currently inactive, suspended or blocked. Recharge not allowed.' });
    }
    ...
    // Verify KSP PIN
    if (!ksp.kspPin || ksp.kspPin.length === 0) {
      return res.status(400).json({ error: 'KSP PIN not set. Please set your PIN first from sidebar.' });
    }
    const isPinValid = await bcrypt.compare(kspPin, ksp.kspPin);
    if (!isPinValid) {
      return res.status(400).json({ error: 'Incorrect KSP PIN. Please try again.' });
    }
...
/**
 * @route   GET /api/ksp/available-cards
 * @desc    Get list of unassigned Nex Cards from inventory
 * @access  Private (KSP only)
 */
router.get('/available-cards', protect, async (req, res) => {
  try {
    const NexCard = require('../models/NexCard');
    const cards = await NexCard.find({ status: 'available' }).select('cardNumber').sort({ cardNumber: 1 });
    res.json(cards);
  } catch (error) {
...
router.post('/generate-card', protect, checkApprovedKsp, async (req, res) => {
  try {
    ...
    if (targetUser.status !== 'approved') {
      return res.status(400).json({ error: `User ka status '${targetUser.status}' hai. Card sirf 'approved' users ko hi allot kiya ja sakta hai.` });
    }
    ...
    // 1. Fetch Charges and Commission from Settings
    const settings = await Settings.getSettings();
    const cardCharge = settings.pricing.nexCardCharges || 0;
    const commissionAmt = settings.commissions.nexCard || 0; // Absolute amount (Rs.)
    ...
    // 3. Check inventory and uniqueness
    const NexCard = require('../models/NexCard');
    const inventoryCard = await NexCard.findOne({ cardNumber });
    
    if (!inventoryCard) {
      return res.status(400).json({ error: 'Card number is not exist' });
    }

    if (inventoryCard.status !== 'available') {
        const assignedToUser = await User.findById(inventoryCard.assignedTo);
        const userName = assignedToUser ? assignedToUser.name : 'someone';
        return res.status(400).json({ error: `This card number is already assigned to ${userName}` });
    }

    const existingUser = await User.findOne({ cardNumber });
    if (existingUser) {
      return res.status(400).json({ error: 'This card number is already assigned to another user' });
    }

    // 4. Update Balances and Card Status
    inventoryCard.status = 'assigned';
    inventoryCard.assignedTo = targetUser._id;
    inventoryCard.assignedAt = new Date();
    await inventoryCard.save();

    // Deduct charge and add commission
    ksp.walletBalance -= cardCharge;
    ksp.walletBalance += commissionAmt;
    
    // Credit Admin (receives charge and pays commission)
    const admin = await User.findOne({ role: 'admin' });
    if (admin) {
      admin.walletBalance = (admin.walletBalance || 0) + cardCharge - commissionAmt;
      await admin.save();
    }
    ...
    // Credit for Admin (cardCharge received)
    if (admin) {
      await Transaction.create({
    ...
    // Credit for KSP (commissionAmt)
    if (commissionAmt > 0) {
      await Transaction.create({
        transactionId: `${txnId}-KSP-C`,
        recipient: ksp._id,
        module: 'KSP',
        amount: commissionAmt,
        type: 'Credit',
        ...
        status: 'Completed',
        performedBy: ksp._id,
        note: `Commission for generating Nex Card for user ${targetUser.name} (Amount: ₹${commissionAmt})`
      });

      // Debit for Admin (commissionAmt paid)
      if (admin) {
        await Transaction.create({
          transactionId: `${txnId}-ADM-D`,
          recipient: admin._id,
          module: 'KSP',
          amount: commissionAmt,
          type: 'Debit',
          ...
          status: 'Completed',
          performedBy: ksp._id,
          note: `KSP Nex Card Commission paid to ${ksp.name} for user ${targetUser.name}`
        });
      }
    }
