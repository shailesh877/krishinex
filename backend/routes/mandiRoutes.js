...
router.get('/', async (req, res) => {
    try {
        const mandis = await Mandi.find({ status: 'active' }).sort({ order: 1, name: 1 });
        res.json(mandis);
    } catch (e) {
...
router.post('/create', protect, async (req, res) => {
    try {
        ...
        const mandi = await Mandi.create({ name, location, order: newOrder });
...
// @route   PATCH /api/mandi/reorder
// @desc    Reorder mandis (Admin only)
// @access  Private
router.patch('/reorder', protect, async (req, res) => {
    try {
        ...
        const updates = items.map(item => 
            Mandi.findByIdAndUpdate(item.id, { order: item.order })
        );
        await Promise.all(updates);
        
        res.json({ message: 'Mandis reordered successfully' });
    } catch (e) {
        console.error('Reorder mandis error:', e);
        res.status(500).json({ error: 'Failed to reorder mandis' });
    }
});
...
router.get('/crops', async (req, res) => {
    try {
        const crops = await Crop.find({ status: 'active' }).sort({ order: 1, name: 1 });
        res.json(crops);
    } catch (e) {
...
router.post('/crops', protect, async (req, res) => {
    try {
        ...
        const crop = await Crop.create({ name, hindiName, icon, order: newOrder });
...
// @route   PATCH /api/mandi/crops/reorder
// @desc    Reorder crops (Admin only)
// @access  Private
router.patch('/crops/reorder', protect, async (req, res) => {
    try {
        const { items } = req.body; // array of { id, order }
        if (!Array.isArray(items)) {
            return res.status(400).json({ error: 'Items array is required' });
        ...
        const updates = items.map(item => 
            Crop.findByIdAndUpdate(item.id, { order: item.order })
        );
        await Promise.all(updates);
        
        res.json({ message: 'Crops reordered successfully' });
    } catch (e) {
        console.error('Reorder crops error:', e);
...
// @desc    Update a Mandi (Admin only)
// @access  Private
router.put('/:id', protect, async (req, res) => {
    try {
        ...
        console.error('Update mandi error:', e);
        res.status(500).json({ error: 'Failed to update mandi' });
    }
});

// @route   PUT /api/mandi/crops/:id
// @desc    Update a Crop (Admin only)
// @access  Private
