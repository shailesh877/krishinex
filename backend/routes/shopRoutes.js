...
// @access  Private
router.post('/items', protect, upload.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 5 }]), async (req, res) => {
    try {
        ...
        if (req.files && req.files['images'] && req.files['images'].length > 0) {
            for (let i = 0; i < req.files['images'].length; i++) {
                ...
                await sharp(file.buffer).resize(500, 500).jpeg({ quality: 90 }).toFile(filepath);
                itemInput.imageUrls.push(`uploads/items/${filename}`);
            }
            itemInput.imageUrl = itemInput.imageUrls[0];
            ...
            itemInput.imageUrls.push(itemInput.imageUrl);
        } else if (req.file) {
            const filename = `item-${Date.now()}.jpg`;
            const filepath = path.join(uploadDir, filename);
            await sharp(req.file.buffer).resize(500, 500).jpeg({ quality: 90 }).toFile(filepath);
            itemInput.imageUrl = `uploads/items/${filename}`;
            itemInput.imageUrls.push(itemInput.imageUrl);
        }
...
// @access  Private
router.put('/items/:id', protect, upload.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 5 }]), async (req, res) => {
    try {
        ...
        if (req.body.imageEdited === 'true') {
            ...
            if (req.files && req.files['images'] && req.files['images'].length > 0) {
                for (let i = 0; i < req.files['images'].length; i++) {
                    ...
                    const filepath = path.join(uploadDir, filename);
                    await sharp(file.buffer).resize(500, 500).jpeg({ quality: 90 }).toFile(filepath);
                    item.imageUrls.push(`uploads/items/${filename}`);
                }
            } else if (req.files && req.files['image'] && req.files['image'].length > 0) {
                const file = req.files['image'][0];
                const filename = `item-${Date.now()}.jpg`;
...
// @route   POST /api/shop/orders/:id/send-delivery-otp
// @desc    Generate a delivery OTP and send to buyer via notification
// @access  Private (Shop Partner)
router.post('/orders/:id/send-delivery-otp', protect, async (req, res) => {
    try {
        const order = await ShopOrder.findById(req.params.id).populate('buyer', 'name phone fcmToken');
        if (!order) return res.status(404).json({ error: 'Order not found' });
        if (order.owner.toString() !== req.user.id) return res.status(401).json({ error: 'Not authorized' });

        // Generate 6-digit OTP and store with 10-min expiry
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
        order.deliveryOtp = otp;
        order.deliveryOtpExpiry = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
        await order.save();

        // Send OTP to buyer via push notification
        const displayId = order._id.toString().slice(-6).toUpperCase();
        await sendNotification(order.buyer._id, {
            title: 'Delivery OTP',
            messageEn: `Your delivery OTP for Order #${displayId} is: ${otp}. Share this with the delivery person to confirm receipt.`,
            messageHi: `ऑर्डर #${displayId} के लिए डिलीवरी OTP है: ${otp}। डिलीवरी कन्फर्म करने के लिए यह OTP दें।`,
            type: 'otp',
            refId: order._id.toString()
        });

        console.log(`[DELIVERY-OTP] Sent OTP ${otp} to buyer ${order.buyer?.name} for order ${displayId}`);
        res.json({ success: true, message: 'OTP sent to buyer' });
    } catch (error) {
...
// @route   POST /api/shop/orders/:id/verify-delivery-otp
// @desc    Verify delivery OTP and mark order as DELIVERED
// @access  Private (Shop Partner)
router.post('/orders/:id/verify-delivery-otp', protect, async (req, res) => {
    try {
        const { otp } = req.body;
        ...
        // OTP is valid - clear it and forward to status update
        order.deliveryOtp = undefined;
        order.deliveryOtpExpiry = undefined;
        await order.save();

        // Now forward to the existing status PATCH logic by calling it internally
        req.body.status = 'DELIVERED';
        res.locals.verifiedDelivery = true;

        // Inline the status update (same logic as PATCH /orders/:id/status for DELIVERED)
        // Re-fetch fresh order
        const freshOrder = await ShopOrder.findById(req.params.id).populate('buyer', 'name phone fcmToken');
        const previousStatus = freshOrder.status;
        freshOrder.status = 'DELIVERED';
        const updatedOrder = await freshOrder.save();

        // Notify buyer
        await sendNotification(freshOrder.buyer._id, {
            title: 'Order Delivered',
            messageEn: `Yay! Your order #${freshOrder._id.toString().slice(-6)} has been delivered successfully.`,
            messageHi: `बधाई हो! आपका ऑर्डर #${freshOrder._id.toString().slice(-6)} सफलतापूर्वक डिलीवर हो गया है।`,
            type: 'status',
            refId: freshOrder._id.toString()
        });

        // Stock deduction + payment logic (same as PATCH)
        if (previousStatus !== 'DELIVERED') {
            try {
                for (const item of freshOrder.items) {
                    ...
                    if (item.variantLabel) {
                        ...
                        const dbItem = await Item.findById(item.itemRef);
                        if (dbItem) {
                            dbItem.stockQty = Math.max(0, (dbItem.stockQty || 0) - qtyToDeduct);
                            await dbItem.save();
                            checkAndNotifyLowStock(dbItem._id).catch(() => {});
                        }
                    }
            ...
            const owner = await User.findById(freshOrder.owner);
            ...
            if (owner) {
                if (freshOrder.paymentMode === 'WALLET') {
                    ...
                    await Transaction.create({
                        transactionId: `SHOP-PAY-${freshOrder._id}-${Date.now()}`,
                        recipient: owner._id, module: 'Shop', amount: payoutAmount,
                        totalAmount, commissionAmount, type: 'Payout', paymentMode: 'NexCard Wallet',
                        status: 'Completed', referenceId: freshOrder._id,
                        note: `Payment for Order #${freshOrder._id.toString().slice(-6)}`
                    });
                } else {
                    owner.walletBalance = (owner.walletBalance || 0) - commissionAmount;
                    await owner.save();
                    await Transaction.create({
                        transactionId: `SHOP-COMM-${freshOrder._id}-${Date.now()}`,
                        recipient: owner._id, module: 'Shop', amount: commissionAmount,
                        totalAmount, type: 'Debit', paymentMode: 'Cash',
                        status: 'Completed', referenceId: freshOrder._id,
                        note: `Admin Commission for Cash Order #${freshOrder._id.toString().slice(-6)}`
                    });
                }
            }
            if (admin && commissionAmount > 0) {
                ...
                await admin.save();
                await Transaction.create({
                    transactionId: `SHOP-ADM-${freshOrder._id}-${Date.now()}`,
                    recipient: admin._id, module: 'Platform', amount: commissionAmount,
                    totalAmount, type: 'Credit', paymentMode: 'NexCard Wallet',
                    status: 'Completed', referenceId: freshOrder._id,
                    note: `Commission from Shop Order #${freshOrder._id.toString().slice(-6)}`
                });
            }
        }
...
// @route   POST /api/shop/pos/search-initiate
// @desc    Initiate farmer search by sending OTP
// @access  Private (Shop Partner Only)
router.post('/pos/search-initiate', protect, async (req, res) => {
    try {
        const { identifier } = req.body;
        if (!identifier) return res.status(400).json({ error: 'Identifier required' });

        // Ensure Shop Partner is verified
        const shopUser = await User.findById(req.user.id);
        if (!shopUser || shopUser.status !== 'approved') {
            return res.status(403).json({ error: 'Only verified Shop Partners can search for farmers.' });
        }
        ...
        if (farmer.status !== 'approved') return res.status(403).json({ error: 'Farmer account is not approved' });
        ...
        console.error('POS Search Initiate error:', error);
        res.status(500).json({ error: 'Failed to initiate search' });
    }
});

// @route   POST /api/shop/pos/search-verify
// @desc    Verify OTP and return farmer details
// @access  Private (Shop Partner Only)
router.post('/pos/search-verify', protect, async (req, res) => {
    try {
        ...
        if (farmer.searchOtp !== otp || new Date() > farmer.searchOtpExpiry) {
            return res.status(400).json({ error: 'Invalid or expired OTP' });
        }

        // Clear OTP after success
        farmer.searchOtp = '';
        farmer.searchOtpExpiry = undefined;
        await farmer.save();
        ...
        console.error('POS Search Verify error:', error);
